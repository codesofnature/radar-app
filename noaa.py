...
# Lower this if you want even faster loading:
# 1.0 = fastest, 1.25 = balanced, 1.5 = sharper but heavier
RADAR_RES_FACTOR = 1.25

RADAR_W = int(WIDTH * RADAR_RES_FACTOR)
RADAR_H = int(HEIGHT * RADAR_RES_FACTOR)

MINUTES_OFFSETS = list(range(0, 18 * 60, 30)) + list(range(18 * 60, 49 * 60, 120))
...
# --- Precomputed Temperature Color LUT ---
...
def fetch_openmeteo_data():
    try:
        ...
        lat_list = [f"{p[0]:.2f}" for p in points]
        lon_list = [f"{p[1]:.2f}" for p in points]
...
def generate_temp_overlay(target_dt, om_data=None):
...
def process_radar_image(img_bytes):
    try:
        ...
        new_a = np.where(
            a < 200,
            np.minimum(a.astype(np.int16) + 55, 255).astype(np.uint8),
            np.uint8(255)
        )
...
def fetch_opensky_planes():
    ...
    headers = {
        "User-Agent": "InstantRadar/1.0 (contact: user@email.com)"
    }
...
<body>
    ...
    <script>
        ...
        const viewBounds = isPhone
            ? [[33.69, -87.63], [41.88, -78.89]]
...
<head>
    ...
    <style>
        ...
        .sun-glow {{
            ...
            height: 160%;
            border-radius: 50%;
            background: radial-gradient(circle, rgba(255, 200, 0, 0.5), rgba(255, 140, 0, 0.3) 40%, transparent 70%);
        ...
        .moon-glow {{
            ...
            height: 140%;
            border-radius: 50%;
            background: radial-gradient(circle, rgba(200, 200, 255, 0.4), transparent 60%);
        ...
        .plane-shadow {{
            ...
            width: 140%;
            height: 20%;
            background: radial-gradient(ellipse, rgba(0, 0, 0, 0.55), rgba(0, 0, 0, 0.2) 40%, transparent 70%);
            border-radius: 50%;
            bottom: -28px;
            opacity: 0.22;
        }}
...
<body>
    ...
    <script id="frames-data" type="application/json">{frames_json}</script>
    ...
    (function() {{
        ...
        function getUvY(el) {{
            ...
            if (jMat) {{
                jMat.uniforms.u_aspect.value = h / w;
            }}
        ...
        const jRenderer = new THREE.WebGLRenderer({{
            canvas: jCanvas,
            alpha: true,
            antialias: true
        }});
        ...
        jMat = new THREE.ShaderMaterial({{
            uniforms: {{
                u_headY: {{ value: 0 }},
                u_tailY: {{ value: 0 }},
                u_aspect: {{ value: 180 / 64 }}
            }},
            vertexShader: `
                varying vec2 vUv;
                void main() {{
            ...
            fragmentShader: `
                ...
                float map(vec2 p) {{
                    return sdCapsule(p, vec2(0.0, u_headY), vec2(0.0, u_tailY), 0.44);
                }}
        ...
        function animateJelly() {{
            headVel = (headVel + (targetY - headY) * 0.04) * 0.85;
            headY += headVel;

            tailVel = (tailVel + (headY - tailY) * 0.03) * 0.80;
            tailY += tailVel;
        ...
        try {{
            frames = JSON.parse(document.getElementById('frames-data').textContent || '[]');
        }} catch (err) {{
            frames = [];
        }}
        ...
        let primaryLayer = L.imageOverlay(
            BLANK,
            activeBounds,
            {{
                pane: 'primaryPane',
        ...
        let tempOverlayLayer = L.imageOverlay(
            BLANK,
            activeBounds,
            {{
                pane: 'tempPane',
        ...
        const sunInd = document.getElementById('sun-indicator');
        const moonInd = document.getElementById('moon-indicator');
        ...
        function tempToColor(f) {{
            let t = Math.max(-60, Math.min(120, f));
            let r, g, b;
        ...
        function updateLabels(gridData) {{
            if (!labelMarkers.length) {{
                gridData.forEach(pt => {{
                    ...
                    let marker = L.marker([pt.lat, pt.lon], {{
                        icon: icon,
                        interactive: false
                    }});
        ...
        function updateAstronomy(date) {{
            ...
            if (sunAltDegrees > -5) {{
                sunInd.style.opacity = Math.min(1, (sunAltDegrees + 5) / 15);
                sunInd.style.transform = `translateY(${{Math.max(0, 30 - sunAltDegrees)}}px)`;
            }} else {{
                sunInd.style.opacity = 0;
            }}
            ...
            if (fraction >= 0.99) {{
                ...
                clipPath = phase <= 0.5
                    ? `ellipse(${{shadowWidth}}% 100% at 0% 50%)`
                    : `ellipse(${{shadowWidth}}% 100% at 100% 50%)`;
            }}
        ...
        function drawFrame(index, skipTrack) {{
            if (!frames || !frames[index]) return;

            primaryLayer.setUrl(frames[index].radarImg || BLANK);
        ...
        (function buildTicks() {{
            ...
            while (t <= lastTs) {{
                ...
                tk.style.left = pct + '%';
                tk.style.height = isMajor ? '8px' : '4px';
        ...
        function swapRandomPlanes() {{
            ...
            selected.forEach((plane, idx) => {{
                const angle = plane.heading - 65;
                const mph = Math.round(plane.velocity * 2.23694);
                const altK = Math.round((plane.altitude * 3.28084) / 1000);

                const planeWrapper =
                    '<div class="plane-wrapper">' +
                        '<div style="position:absolute;top:-25px;background:rgba(255,255,255,0.05);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);padding:2px 8px;border-radius:6px;border:1px solid rgba(0,0,0,0.1);font-size:11px;font-weight:800;color:#0f172a;white-space:nowrap;pointer-events:none;z-index:10;">' +
                            plane.callsign + ' • ' + mph + ' mph • ' + altK + 'k ft' +
                        '</div>' +
                        '<div class="plane-shadow"></div>' +
                        '<span style="font-size:32px;transform:rotate(' + angle + 'deg);filter:drop-shadow(3px 5px 4px rgba(0,0,0,0.4));line-height:1;">✈️</span>' +
                    '</div>';
        ...
        function animatePlanes() {{
            ...
            planeMarkers.forEach(pm => {{
                ...
                pm.marker.setLatLng([
                    pm.plane.lat + latDrift,
                    pm.plane.lon + lonDrift
                ]);
            }});
        ...
        dolphinPositions.forEach((pos, idx) => {{
            ...
            const dolphinIcon = L.divIcon({{
                className: 'dolphin-container',
                html: dolphinWrapper,
                iconSize: [20, 20],
                iconAnchor: [10, 10]
            }});

            const marker = L.marker(pos.center, {{
                icon: dolphinIcon,
                interactive: false,
                zIndexOffset: 800
            }}).addTo(map);
        ...
        function hideLoadingOverlay() {{
            ...
            if (overlay && !overlay.dataset.hidden) {{
        ...
        if (totalFrames > 0) {{
            ...
            firstImg.onload = hideLoadingOverlay;
            firstImg.onerror = hideLoadingOverlay;
            firstImg.src = frames[0].radarImg || BLANK;
...
def render_flipbook():
    ...
    # Phase 1: show map immediately
    with map_slot:
        components.html(
            generate_map_shell_html(),
            height=850,
            scrolling=False
        )
    ...
    if accumulated:
        ...
        map_slot.empty()
        map_slot.error("Failed to load radar data. Please refresh.")
