...
.temp-label {{
    ...
    font-size: 20px; font-weight: 200; letter-spacing: 0.2px; text-align: center;
...
#layer-selector {{
    ...
    padding: 12px 18px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.15);
    display: flex; flex-direction: column; gap: 12px; font-size: 15px; font-weight: 700; color: #0f172a;
}}
...
#left-controls {{
    ...
    background: rgba(255, 255, 255, 0.15); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px);
    padding: 15px 12px; border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.15);
...
#plane-shadow {{
    ...
    bottom: -4px; opacity: 0.85;
}}

/* --- YACHT ANIMATION CSS (3/4 Perspective) --- */
.yacht-container {{ pointer-events: none; }}
#yacht-icon {{ width: 100%; height: 100%; transition: filter 2s ease-out; filter: drop-shadow(-2px 3px 3px rgba(0,0,0,0.5)); }}
.yacht-wake {{
    ...
    border-radius: 50%; animation: wakePulse 1.8s ease-in-out infinite; pointer-events: none;
}}
@keyframes wakePulse {{
...
// --- AIRPLANE ANIMATION JS (with ground shadow) ---
const planeStart = [33.94, -118.40];
const planeEnd = [40.64, -73.77];
const dy = planeEnd[0] - planeStart[0];
const dx = planeEnd[1] - planeStart[1];
const planeAngle = 90 - (Math.atan2(dy, dx) * 180 / Math.PI);
const planeSvg = `
<div id="plane-wrapper" style="transform: rotate(${{planeAngle}}deg); width: 100%; height: 100%; position: relative;">
    <div id="plane-shadow"></div>
    <img src="data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'><defs><linearGradient id='gloss' x1='0' y1='0' x2='1' y2='1'><stop offset='0' stop-color='%23777777'/><stop offset='0.3' stop-color='%23151515'/><stop offset='1' stop-color='%23000000'/></linearGradient></defs><path fill='url(%23gloss)' stroke='%23333333' stroke-width='5' stroke-linejoin='round' d='M256,100 L16,300 L60,320 L130,240 L190,300 L230,250 L256,280 L282,250 L322,300 L382,240 L452,320 L496,300 Z'/></svg>" id="flying-plane" class="plane-taking-off" style="width:100%;height:100%;"/>
</div>`;
...
function animateFlight() {{
    ...
    if (progress > 1) {{
        ...
        if (shadowEl) {{ shadowEl.style.bottom = '-28px'; shadowEl.style.opacity = '0.22'; shadowEl.style.width = '140%'; }}
    }}
...
// --- YACHT ANIMATION JS (3/4 Perspective Side View) ---
const yachtSvg = `
<div style="transform: rotate(0deg); width: 100%; height: 100%;" id="yacht-wrapper">
    <div class="yacht-wake"></div>
    <svg id="yacht-icon" xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 96'>
        <defs>
            <linearGradient id='yHullGrad' x1='0' y1='0' x2='1' y2='0'>
                <stop offset='0' stop-color='#e8e8e8'/>
                <stop offset='0.4' stop-color='#ffffff'/>
                <stop offset='1' stop-color='#b8b8b8'/>
            </linearGradient>
            <linearGradient id='yDeckGrad' x1='0' y1='0' x2='0' y2='1'>
                <stop offset='0' stop-color='#f5e6c8'/>
                <stop offset='1' stop-color='#c9a876'/>
            </linearGradient>
        </defs>
        <ellipse cx='32' cy='86' rx='24' ry='5' fill='rgba(0,0,0,0.3)'/>
        <path fill='url(#yHullGrad)' stroke='#2c3e50' stroke-width='1.2' stroke-linejoin='round'
              d='M32,6 C24,14 14,38 13,60 C12,74 20,82 32,84 C44,82 52,74 51,60 C50,38 40,14 32,6 Z'/>
        <path fill='rgba(255,255,255,0.35)'
              d='M32,6 C24,14 14,38 13,60 C12,74 20,82 32,84 L32,6 Z'/>
        <path d='M14,60 Q32,66 50,60' stroke='#1e3a5f' stroke-width='1.5' fill='none'/>
        <path fill='#6b1010' opacity='0.75'
              d='M13,60 C12,74 20,82 32,84 C44,82 52,74 51,60 Q32,66 13,60 Z'/>
        <path fill='url(#yDeckGrad)' stroke='#8b6f47' stroke-width='0.6'
              d='M26,18 C22,28 17,46 16,58 Q32,64 48,58 C47,46 42,28 38,18 Q32,13 26,18 Z'/>
        <path fill='#4a6fa5' stroke='#2c3e50' stroke-width='0.6'
              d='M25,32 L25,54 Q32,57 39,54 L39,32 Q32,29 25,32 Z'/>
        <path fill='#6a9fd5' stroke='#2c3e50' stroke-width='0.3'
              d='M25,32 Q32,29 39,32 Q32,35 25,32 Z'/>
        <rect x='27' y='38' width='3' height='2.5' rx='0.5' fill='#a8d5ff' stroke='#2c3e50' stroke-width='0.2'/>
        <rect x='34' y='38' width='3' height='2.5' rx='0.5' fill='#a8d5ff' stroke='#2c3e50' stroke-width='0.2'/>
        <line x1='32' y1='32' x2='32' y2='14' stroke='#2c3e50' stroke-width='0.9'/>
        <circle cx='32' cy='13' r='1.3' fill='#e74c3c'/>
        <path d='M28,16 Q32,12 36,16' stroke='#c0c0c0' stroke-width='0.6' fill='none'/>
    </svg>
</div>`;

const yachtIcon = L.divIcon({{
    className: 'yacht-container',
    html: yachtSvg,
    iconSize: [40, 40],
    iconAnchor: [20, 20]
}});

const yachtStart = [37.5, -73.0];
const yachtMarker = L.marker(yachtStart, {{
    icon: yachtIcon,
    interactive: false,
    zIndexOffset: 800
}}).addTo(map);

const yachtCenter = [38.5, -72.5];
const yachtRadiusLat = 3.5;
const yachtRadiusLon = 2.0;
const yachtCycleMs = 90000;
const yachtStartTime = Date.now();

function animateYacht() {{
    const elapsed = Date.now() - yachtStartTime;
    const t = (elapsed % yachtCycleMs) / yachtCycleMs;
    const angle = t * 2 * Math.PI;

    const lat = yachtCenter[0] + yachtRadiusLat * Math.sin(angle);
    const lon = yachtCenter[1] + yachtRadiusLon * Math.cos(angle);

    const dLat = yachtRadiusLat * Math.cos(angle);
    const dLon = -yachtRadiusLon * Math.sin(angle);
    const headingDeg = 90 - (Math.atan2(dLon, dLat) * 180 / Math.PI);

    yachtMarker.setLatLng([lat, lon]);
    const wrapper = document.getElementById('yacht-wrapper');
    if (wrapper) {{
